...
C   If X is a terminal then FIRST(X) = {X} Example: F -> I | id
We can write it as FIRST(F) -> { ( , id )
D   If X is a non-terminal like E -> T then to get FIRSTI substitute T with other productions until you get a terminal as the first symbol
E   If X -> ε then add ε to FIRST(X).


For computing the follow:
13. Always check the right side of the productions for a non-terminal, whose FOLLOW set is being found. (never see the left side).
14. (a) If that non-terminal (S,A,B…) is followed by any terminal (a,b…,*,+,(,)…) , then add that terminal into the FOLLOW set.
